    ...
    <>
      ...
      >
        ...
        <header className="flex items-center justify-between px-7 py-6 border-b border-[#1A1A1A]/8">
          <div>
            <p className="text-[10px] uppercase tracking-[0.3em] text-[#A68A64] font-bold mb-1">Tu selección</p>
            <h2 className="font-serif text-2xl text-[#1A1A1A] leading-none m-0">
              Pedido {count > 0 && <span className="text-[#A68A64]">({count})</span>}
            </h2>
          </div>
          <button
            onClick={() => closeCart()}
            aria-label="Cerrar"
            className="p-2 text-[#1A1A1A] hover:text-[#A68A64] transition-colors"
          >
          ...
          {isEmpty ? (
            <div className="h-full flex flex-col items-center justify-center text-center px-4">
              <div className="w-16 h-16 rounded-full bg-white flex items-center justify-center mb-6 shadow-sm">
                <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#A68A64" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round">
              ...
              <a
                href="/productos"
                onClick={() => closeCart()}
                className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#1A1A1A] border-b border-[#1A1A1A]/20 pb-1 hover:border-[#A68A64] hover:text-[#A68A64] transition-all"
              >
                Ver nuestros cafés
              </a>
            </div>
                ...
                <li key={it.id} className="flex gap-4 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <a
                      href={`/products/${it.id}`}
                      onClick={() => closeCart()}
                      className="font-serif text-[17px] text-[#1A1A1A] leading-tight hover:text-[#A68A64] transition-colors block truncate"
                    >
                      {it.name}
                    </a>
                    ...
                    <div className="flex items-center justify-between mt-2.5">
                      ...
                      <div className="inline-flex items-center border border-[#1A1A1A]/15 rounded-sm">
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center text-[#1A1A1A] hover:text-[#A68A64] transition-colors"
                        >
                          −
                        </button>
